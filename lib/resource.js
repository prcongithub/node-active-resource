...
Resource.customPost = function(options){
  return function(request, conditions, post_data, callback){
    ...
    var uri = Paths.generateURL(options.url, conditions) + "?" + Qs.stringify(conditions, { arrayFormat: 'brackets' });
    var root = this.root();
...
Resource.create = function(request, conditions, attributes, callback){
  var conditions = conditions ? conditions : {};
  var uri = this.requestPath(conditions) + "?" + Qs.stringify(conditions, { arrayFormat: 'brackets' });
  var root = this.root();
  var object = {};
...
Resource.prototype.save = function(request, conditions, attributes, callback){
  ...
  var uri = this.collectionPath() + "/" + this.id + "?" + Qs.stringify(this.conditions, { arrayFormat: 'brackets' });
  var root = this.root();
  var object = {};
  object[root] = attributes;
  var post_data = Qs.stringify(object, { indices: true });
