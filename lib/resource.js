module.exports = function(api_options) {
  ...
  Resource.define_resource = function(options){
    ...
    Klass.host = api_options[use_api].host;
    Klass.port = api_options[use_api].port;
    return Klass;
  };
  ...
  Resource.customPost = function(options){
    return function(request, conditions, post_data, callback){
      ...
      http.post(request, {
        uri: uri,
        host: api_options[options.use_api || this.use_api].host,
        port: api_options[options.use_api || this.use_api].port
      }, post_data, function(response,error){
        var object = new klass(response);
        callback(object,error);
      });
    }
  ...
  Resource.customMemberGet = function(options){
    return function(request, conditions, callback){
      ...
      http.get(request, {
        uri: uri,
        host: api_options[options.use_api || klass.use_api].host,
        port: api_options[options.use_api || klass.use_api].port
      }, function(data, error){
        if(error) {
          callback(null, error);
        } else {
