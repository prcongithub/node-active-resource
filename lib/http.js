        ...
        if (api_res.statusCode <= 210 || api_res.statusCode == 422) {  
          ...
          api_res.on("end",function(chunk) {
            try{
              ...
              //callback(null, new errors.ClientError(JSON.parse(data).error,api_res.statusCode));
              var clientError = new errors.ClientError(errorBody.error,api_res.statusCode);
       	      callback(null, clientError);
            } catch(e){
              console.log(data);
              var clientError = new errors.ClientError("ParseError "+e.message,api_res.statusCode)
              callback(null,clientError);
       	    } 
          });
        }  
